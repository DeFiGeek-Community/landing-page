...
監査完了まで詳細は伏せますが、レビュー修正を含む監査結果は随時公開いたします。

最終的な監査完了時期は9月～10月が想定されます。
...
プロジェクトの性質上複数のモデルを提供可能ですので、モデル・キャラクター等をWalletアバターNFTに組み込みたい、
...
コミュニティダッシュボード構築の第一弾として、TXJPのホルダー推移等の統計情報を確認できるアナリティクスサイトの検討・設計・実装を開始しました。
...
現在はプライベートリポジトリにて開発・動作確認を行っていますが、クオリティの担保とソースコードのライセンスの確認が取れ次第OSSとして公開され、コミットが可能になります。
